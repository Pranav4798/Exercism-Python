...
def compare_records(azara_record, rui_record):
    ...
    coord = tuple(azara_record[1])
    if coord == rui_record[1]:
        return True
    else:
        return False
...
def clean_up(combined_record_group):
    ...
    for record in combined_record_group:
        row = (record[0], record[2], record[3], record[4])
        clean_rec.append(str(row))
    return "\n".join(clean_rec) + "\n"

# print(clean_up((('Brass Spyglass', '4B', 'Abandoned Lighthouse', ('4', 'B'), 'Blue'), 
